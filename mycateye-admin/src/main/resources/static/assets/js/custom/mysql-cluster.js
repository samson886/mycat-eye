...
var fnLoadClusterList = function() {
	...
	$.getJSON(ajaxUrl, function(json) {
		...
		$.each(json, function(i, value) {
			...
			html += "</td>";
			html += "<td>";
			html += "<button type=\"button\" onClick=\"javascript:fnEditCluster(" + value.id + ",'" + value.name + "')\" class=\"btn btn-default btn-sm oper-btn\"><span class=\"glyphicon glyphicon-edit\"></span> 编辑</button>&nbsp;&nbsp;&nbsp;";
			html += "<button type=\"button\" onClick=\"javascript:fnDeleteCluster(" + value.id + ",'" + value.name + "')\" class=\"btn btn-default btn-sm oper-btn\"><span class=\"glyphicon glyphicon-remove\"></span> 删除</button>";
			html += "</td>";
			html += "</tr>";
		});
